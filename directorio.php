                ...
                <div class=" col-lg-9">
                    <div class="binduz-er-author-item mb-40">
                        <div class="binduz-er-content">
                            <h3 class="binduz-er-title">Directorio de servidores públicos, empleados o contratistas</h3>
                        </div>
                    </div>
                </div>
						<table align="center" border="0" style="width:99%">	
							<thead>		
								<tr>
									<th>Nombre</th>
									<th>Posición</th>
							...
							<tbody>
								<?php $ssqldir = "Select * from directorio WHERE publicar = 1 ORDER BY nombre";
											$resultdir = $conn->query( $ssqldir );
											while ( $rowdir = $resultdir->fetch_assoc() ) { ?>
								<tr>
									<td><?php echo $rowdir["nombre"] ?></td>
									<td><?php echo $rowdir["cargo"] ?></td>
									<td><?php echo $rowdir["email"] ?></td>
									<td><?php echo $rowdir["telefono"] ?></td>
								</tr>
								<?php } ?>
							</tbody>
						</table>
						...
						<h3>Adjuntos:</h3>
									...
									<tbody>
									<?php $ssqladj = "Select * from adjuntos WHERE num_id = '163' ORDER BY prioridad";
											$resultadj = $conn->query( $ssqladj );
											while ( $rowadj = $resultadj->fetch_assoc() ) { $tipoarchivo = $rowadj['tipo']; ?>
									<tr><th><?php echo $rowadj['nombre'] ?></th><th><?php echo $rowadj['descripcion'] ?></th><th><a href="<?php echo $rowadj['vinculo'] ?>" target="_blank"><img src="assets/images/<?php if($tipoarchivo == 'pdf') { echo "iconopdf.jpg"; } elseif($tipoarchivo == 'xls') { echo "iconoxls.jpg"; } elseif($tipoarchivo == 'xlsx') { echo "iconoxls.jpg"; } ?>"> Ver archivo</a></th><th><?php echo $rowadj['tamano'] ?> KB</th><th style="font-size: 11px; font-style: normal">Creado el <?php echo $rowadj['fecha_publicado'] ?> <br>Última actualización <?php echo $rowadj['fecha_publicado'] ?></th></tr>
									<?php } ?>
									</tbody>
									</table>
