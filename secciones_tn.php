...
<body>
                ...
                <div class="col-lg-12">
                    <div class="binduz-er-content">
                        <div class="binduz-er-video-post-title">
                            <h3 class="binduz-er-title"><?php $ssqlsecc = "Select * from secciones WHERE slug = '".$_GET['seccion']."'";

	//Ejecuto la consulta 

	$resultsecc = $conn->query( $ssqlsecc );

	while ( $rowsecc = $resultsecc->fetch_assoc() ) {
								echo $rowsecc['nombre']; } ?></h3>
                        </div>
                    </div>
                    <div class="row">
  ...
  <?php $ssqlmen = "Select * from subsecciones WHERE seccion = '".$_GET['seccion']."' AND publicar = 1 ORDER BY prioridad";
  ...
  $ssqltiene = "Select seccion,titulo,slug,prioridad,en_menu,publicar,tieness,vinculo,id from articulos WHERE subseccion = '$subseccion' AND publicar = 1 AND tieness = 0 ORDER BY prioridad LIMIT 1";
	...
	while ( $rowtiene = $resulttiene->fetch_assoc() ) { $elvinculo = $rowtiene['vinculo']; $tieness = $rowtiene['tieness']; ?>
	  <li class="nav-item active"><a class="nav-link" href="intro.php?seccion=<?php echo $rowmen['seccion'] ?>&slug=<?php echo $rowmen['slug'] ?>"><i class="fa fa-angle-down"></i> <?php echo $rowmen['prioridad'] ?>. <?php echo $txsubseccion ?></a>
			<ul class="sub-menu"><?php $ssqlsub = "Select seccion,titulo,slug,prioridad,en_menu,publicar,tieness,vinculo,blank,id,tercer_nivel from articulos WHERE subseccion = '$subseccion' AND publicar = 1 ORDER BY prioridad";
	$resultsub = $conn->query( $ssqlsub );
	while ( $rowsub = $resultsub->fetch_assoc() ) { $vinculosub = $rowsub['vinculo']; $blanksub = $rowsub['blank']; $tercer_nivel = $rowsub['tercer_nivel']; if ($vinculosub == '') { ?>
			<li><a href="contenido.php?id=<?php echo $rowsub['id'] ?>&seccion=<?php echo $rowsub['seccion'] ?>&slug=<?php echo $rowsub['slug'] ?>"><?php echo $rowmen['prioridad'] ?>.<?php echo $rowsub['prioridad'] ?>. <?php echo $rowsub['titulo'] ?></a>
				<?php if($tercer_nivel == 1) { ?>
				<ul class="sub-menu">
					<?php $ssqltn = "Select seccion,titulo,slug,prioridad,en_menu,publicar,tieness,vinculo,blank,id,tercer_nivel from articulos WHERE subseccion = '$subseccion' AND publicar = 1 ORDER BY prioridad";
	$resulttn = $conn->query( $ssqltn );
	while ( $rowtn = $resulttn->fetch_assoc() ) { $vinculotn = $rowtn['vinculo']; $blanktn = $rowtn['blank']; if ($vinculosub == '') { ?>
					<li><a href="contenido.php?id=<?php echo $rowtn['id'] ?>&seccion=<?php echo $rowtn['seccion'] ?>&slug=<?php echo $rowtn['slug'] ?>"><?php echo $rowmen['prioridad'] ?>.<?php echo $rowsub['prioridad'] ?>.<?php echo $rowtn['prioridad'] ?>. <?php echo $rowsub['titulo'] ?></a></li>
					<?php } else { ?>
