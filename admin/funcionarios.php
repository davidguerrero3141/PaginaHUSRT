                        ...
                        <div class="card r-0 shadow">
								<table class="table table-bordered">
                                    ...
                                    <tr>
										...
										<?php $ssql = "Select * from directorio";
								$result = $conn->query($ssql);
      							...
      							while($row = $result->fetch_assoc()) { if ($row["publicar"] == 1) {$publicado = 'No publicar' & $button = "btn btn-success btn-sm";} elseif ($row["publicar"] == 0) {$publicado = 'Publicar' & $button = "btn btn-warning btn-sm";} ?>
                                    <tr>
										...
										<td><a class="<?php echo $button ?>" href="publicar.php?tabla=directorio&id=<?php echo $row["id"] ?>&publicar=<?php echo $row["publicar"]?>"><?php if ($row["publicar"] == 0) { echo "No publicado";} else { echo "Publicado"; }?></a></td>
										<td><a class="btn btn-danger btn-sm" href="borrado.php?tabla=directorio&id=<?php echo $row["id"]?>" onclick='clicked(event)'>Borrar</a></td>
                                    </tr>
                                    <?php }} else { echo ""; } ?>
                                    </tbody>
                                </table>
							<div class="card-body b-b">
							<form action="agregado_funcionario.php" method="post" enctype="multipart/form-data">
                            <h2>Agregue un nuevo funcionario:</h2>
                            <div class="form-group">
								<label for="nombre" class="col-form-label">Nombre</label>
                                <input class="form-control form-control-lg" type="text" id="nombre" name="nombre" required>
                            </div>
							<div class="form-group">
								<label for="usuario" class="col-form-label">Posición</label>
                                <input class="form-control form-control-lg" type="text" id="cargo" name="cargo" required>
                            </div>
							<div class="form-group">
								<label for="email" class="col-form-label">Email</label>
                                <input class="form-control form-control-lg" type="text" id="email" name="email" required>
                            </div>
							<div class="form-group">
								<label for="clave" class="col-form-label">Teléfono</label>
                                <input class="form-control form-control-lg" type="text" id="telefono" name="telefono" required>
                            </div>
