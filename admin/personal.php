                    ...
                    <div class="col-md-12">
                        <div class="card r-0 shadow">
      							...
      							while($row = $result->fetch_assoc()) { if ($row["publicar"] == 1) {$publicado = 'No publicar' & $button = "btn btn-success btn-sm";} elseif ($row["publicar"] == 0) {$publicado = 'Publicar' & $button = "btn btn-warning btn-sm";}?>
                                    <tr>
                                        <td><form action='prioridad.php' method="post">
												...
												<input name="tabla" type="hidden" id="tabla" value="personal"/>
												<input name="submit" class="button-grey arrow" type="submit" value="Cambiar"/></form></td>
										<td><? echo $row["nombre"]; ?> <? echo $row["apellido"]; ?></td>
										<td><img src="<? echo $row["imagen"]; ?>" width="50px"></td>
										<td><? echo $row["email"]; ?></td>
										<td><? echo $row["telefono"]; ?></td>
										<td><? echo $row["whatsapp"]; ?></td>
										<td><a class="<? echo $button ?>" href="publicar.php?tabla=personal&id=<? echo $row["id"] ?>&publicar=<? echo $row["publicar"]?>">Publicar</a></td>
										<td><a class="btn btn-danger btn-sm" href="borrado.php?tabla=personal&id=<? echo $row["id"]?>" onclick='clicked(event)'>Borrar</a></td>
                                    </tr>
                                    <? }} else { echo ""; } ?>
                                    </tbody>
                                </table>
							<div class="card-body b-b">
							<form action="agregado_personal.php" method="post" enctype="multipart/form-data">
                            <h2>Agregue un nuevo agente:</h2>
							...
							<div class="form-group">
                                ...
                                <input class="form-control form-control-lg" type="text" id="apellido" name="apellido" required>
                            </div>
							<div class="form-group">
							...
							<div class="form-group">
                                ...
                                <input class="form-control form-control-lg" type="text" id="whatsapp" name="whatsapp" required>
                            </div>
