                ...
                <div class="col-md-7">
                    <div class="card">
                        ...
                        <div class="card-body b-b">
							<table class="table table-bordered">
                                    ...
                                    <tr>
										...
										<?php $ssql = "Select * from slider ORDER BY prioridad";
      							...
      							while($row = $result->fetch_assoc()) { if ($row["publicar"] == 1) {$publicado = 'No publicar' & $button = "btn btn-success btn-sm";} elseif ($row["publicar"] == 0) {$publicado = 'Publicar' & $button = "btn btn-warning btn-sm";}?>
                                    <tr>
                                        <td><form action='prioridad.php' method="post">
												...
												<input name="tabla" type="hidden" id="tabla" value="slider"/>
												<input name="submit" class="button-grey arrow" type="submit" value="Cambiar"/></form></td>
                                        <td><img src="../..<?php echo $row["imagen"]; ?>" width="250px"></td>
                                        <td><a class="<?php echo $button ?>" href="publicar.php?tabla=slider&id=<?php echo $row["id"] ?>&publicar=<?php echo $row["publicar"]?>">Publicar</a></td>
										<td><a class="btn btn-danger btn-sm" href="borrado.php?tabla=slider&id=<?php echo $row["id"]?>" onclick='clicked(event)'>Borrar</a></td>
                                    </tr>
                                    <?php }} else { echo ""; } ?>
                                    </tbody>
                                </table>
						...
						<div class="card-body b-b">
							...
							<form enctype="multipart/form-data" action="agregar_slider.php" method="POST">
...
<input  type="button" value="Seleccionar imagen" onclick="BrowseServer();" />
							...
							<input type="hidden" name="tabla" value="slider" />
							<input type="hidden" name="pagina" value="slider" />
                            <p>
                ...
                <div class="col-md-5">
                    <h3>Documentación</h3>
                    <hr>
                    <p>El tamaño ideal para las imágenes es de 1920 x 700 pixels.</p>
                </div>
            </div>
        </div>
    </div>
