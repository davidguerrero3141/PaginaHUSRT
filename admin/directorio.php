                        ...
                        <div class="card-body b-b">
							<table class="table table-bordered">
                                    ...
                                    <tr>
										...
										<? $ssql = "Select * from directorio where tipo = '".$_GET['tipo']."' ORDER BY prioridad";
								$result = $conn->query($ssql);
  								if ($result->num_rows > 0) {
      							while($row = $result->fetch_assoc()) { if ($row["publicar"] == 1) {$publicado = 'No publicar' & $button = "btn btn-success btn-sm";} elseif ($row["publicar"] == 0) {$publicado = 'Publicar' & $button = "btn btn-warning btn-sm";} if ($row["destacado"] == 1) {$destacado = 'No destacar' & $buttondes = "btn btn-success btn-sm";} elseif ($row["destacado"] == 0) {$destacado = 'Destacar' & $buttondes = "btn btn-warning btn-sm";}?>
                                    <tr>
                                        <td><form action='prioridad.php' method="post">
												...
												<input name="tabla" type="hidden" id="tabla" value="directorio"/>
												<input name="submit" class="btn btn-primary" type="submit" value="Cambiar"/></form></td>
										<td><? echo $row["tiponegocio"]; ?></td>
										<td><? echo $row["ciudad"]; ?></td>
										<td><? echo $row["nombre"]; ?></td>
                                        <td><a href="actualizar_dir.php?id=<? echo $row["id"] ?>" class="btn btn-primary btn-sm">ACTUALIZAR</a></td>
										<td><? if ($row["galeria"] == 1) { ?><a href="galeria.php?id_gal=<? echo $row["id"] ?>" class="btn btn-primary btn-sm">GALERIA</a><? } else { echo ""; } ?></td>
                                        <td><a class="<? echo $button ?>" href="publicar.php?tabla=directorio&id=<? echo $row["id"] ?>&publicar=<? echo $row["publicar"]?>">Publicar</a></td>
										<td><a class="<? echo $buttondes ?>" href="destacar.php?tabla=directorio&id=<? echo $row["id"] ?>&destacado=<? echo $row["destacado"]?>">Destacado</a></td>
										<td><a class="btn btn-danger btn-sm" href="borrado.php?tabla=directorio&id=<? echo $row["id"]?>" onclick='clicked(event)'>Borrar</a></td>
                                    </tr>
                                    <? }} else { echo ""; } ?>
                                    </tbody>
                                </table>
