...
<div class="page has-sidebar-left">
    ...
    <div class="container-fluid my-3">
        <p>Aquí podrá actualizar y modificar el contenido de los aliados</p>
    </div>
                ...
                <div class="col-md-7">
                    <div class="card">
                        ...
                        <div class="card-body b-b">
							<table class="table table-bordered">
                                    ...
                                    <tr>
										...
										<?php $ssql = "Select * from aliados ORDER BY prioridad";
      							...
      							while($row = $result->fetch_assoc()) { if ($row["publicar"] == 1) {$publicado = 'No publicar' & $button = "btn btn-success btn-sm";} elseif ($row["publicar"] == 0) {$publicado = 'Publicar' & $button = "btn btn-warning btn-sm";}?>
                                    <tr>
                                        <td><form action='prioridad.php' method="post">
												...
												<input name="tabla" type="hidden" id="tabla" value="aliados"/>
												<input name="submit" class="button-grey arrow" type="submit" value="Cambiar"/></form></td>
                                        <td><?php echo $row["nombre"]; ?></td>
										<td><img src="../..<?php echo $row["imagen"]; ?>" width="250px"></td>
                                        <td><a class="<?php echo $button ?>" href="publicar.php?tabla=aliados&id=<?php echo $row["id"] ?>&publicar=<?php echo $row["publicar"]?>">Publicar</a></td>
										<td><a class="btn btn-danger btn-sm" href="borrado.php?tabla=aliados&id=<?php echo $row["id"]?>" onclick='clicked(event)'>Borrar</a></td>
                                    </tr>
                                    <?php }} else { echo ""; } ?>
                                    </tbody>
                                </table>
						...
						<div class="card-body b-b">
							...
							<form enctype="multipart/form-data" action="agregar_aliado.php" method="POST">
...
<input  type="button" value="Seleccionar imagen" onclick="BrowseServer();" />
							...
							<input type="hidden" name="tabla" value="aliados" />
							<input type="hidden" name="pagina" value="aliados" />
                            <p>
                ...
                <div class="col-md-5">
                    <h3>Documentación</h3>
                    <hr>
                    <p>Los aliados aparecen en la parte inferior de la página antes del footer</p>
					<p><img src="assets/img/aliados.jpg"></p>
                </div>
            </div>
        </div>
    </div>
